...
int readhisto()
{
  ...
  ofstream myfile;
  myfile.open("pred_hist_extractor.txt");
  // list of variables
  ...
  for(int v = 0; v < 2; v++)
  {
	...
	myfile<<string(50, '*').c_str()<<endl<<"*** NLO prediction mt = "<<mt[i]<<endl;
      double total = 0.0;
	...
	myfile<<h500k->GetBinLowEdge(b + 1)<<"<"<<titles[v].Data()<<"<"<<h500k->GetBinLowEdge(b + 2)<<", x_sec_pred = "<<xsec<<endl;
      }
...
myfile<<"*** Total x-section_pred = "<<total<<endl<<"Integrated x-sec_pred = "<<h500k->Integral(1, h500k->GetNbinsX(), "width") * 5e5<<endl;
